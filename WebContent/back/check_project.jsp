		...
		<tbody id="myTbody">
			<c:choose>
				<c:when test="${not empty requestScope.projectList}">
					<c:forEach items="${requestScope.projectList}" var="project" varStatus="st">
						<tr>
							<td>${st.count}</td>
							<td>${project.projectName}</td>
							<td>${project.deptName}</td>
							<td>
								<button type="button" class="btn btn-primary" onclick="updateResult(${project.projectId},${requestScope.examineId},${project.deptName})">体检小结</button>
							</td>
						</tr>
					</c:forEach>
				</c:when>
...
<script>
	//更改小结
	function updateResult(projectId,examineId,deptName){
